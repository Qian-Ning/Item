...
```js
js复制代码// WeakMap常用于存储只有当key所引用的对象存在时（没有被回收）才有价值的消息，十分贴合双向绑定场景
...
```js
js复制代码componentWillReciveProps(props改变)/getDrivedStateFromProp ->
...
```js
js复制代码currentFiber.alternate = workInProgressFiber
...
```js
js复制代码const PROMISE_STATUS_PENDING = "pending";
...
```ts
ts复制代码function debounce(fn: Function, delay: number) {
  let timer: any = null;
...
```ts
ts复制代码function quickSort(arr: number[], startIndex = 0): number[] {
...
```ts
ts复制代码function dealArr(arr1: any[], arr2: any[]): any[] {
...
```ts
ts复制代码function convert(money: number): string {
